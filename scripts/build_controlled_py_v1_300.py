#!/usr/bin/env python3
"""Build controlled_py_v1_300 with public tests and external hidden tests."""
...
DEFAULT_WORKSPACE_ROOT = PROJECT_ROOT / "workspaces" / "controlled_py_v1_300"
DEFAULT_HIDDEN_ROOT = PROJECT_ROOT / "data" / "hidden_tests" / "controlled_py_v1_300"
DEFAULT_TASKS = PROJECT_ROOT / "data" / "tasks" / "controlled_py_v1_300.jsonl"
DEFAULT_REPORT = PROJECT_ROOT / "results" / "controlled_py_v1_300_build_report.json"
DEFAULT_MD_REPORT = PROJECT_ROOT / "reports" / "10_controlled_py_v1_300_report.md"
...
def make_task(
    *,
    ...
    return GeneratedTask(
        task_id=f"controlled_py_v1_f{family_index:02d}_{stem}_{variant:03d}",
...
def write_task(workspace_root: Path, hidden_root: Path, task: GeneratedTask) -> dict[str, Any]:
    ...
    return {
        ...
        "repo": "controlled/python-mini-repair-v1",
        "language": "Python",
        "difficulty": task.difficulty,
        "split": task.split,
        "source_dataset": "controlled_py_v1_300",
        "base_commit": "local",
        ...
        "metadata": {
            ...
            "split_policy": "per_family_11_train_2_val_2_heldout",
        },
    }
...
def write_markdown_report(path: Path, report: dict[str, Any]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    text = f"""# Controlled Python Mini-Repo Benchmark v1 300

Date: 2026-07-04

## Result

`controlled_py_v1_300` expands the initial 30-task benchmark to 300 deterministic Python code-repair tasks with separated public and hidden tests.
...
```text
agentic-rl/data/tasks/controlled_py_v1_300.jsonl
agentic-rl/data/tasks/controlled_py_v1_300_train.jsonl
agentic-rl/data/tasks/controlled_py_v1_300_val.jsonl
agentic-rl/data/tasks/controlled_py_v1_300_heldout_test.jsonl
agentic-rl/data/hidden_tests/controlled_py_v1_300/
agentic-rl/workspaces/controlled_py_v1_300/
agentic-rl/results/controlled_py_v1_300_build_report.json
```

## Design

The benchmark contains 20 generation families with 15 variants per family. They map to 19 bug-family labels because two generation families are path-handling variants. Each generation family contributes 11 train tasks, 2 validation tasks, and 2 held-out test tasks, so the held-out set has 40 tasks and every generation family appears in held-out evaluation.

Public tests live inside each task workspace under `tests/` and are the tests an agent may run during repair. Hidden tests live outside the workspace under `agentic-rl/data/hidden_tests/controlled_py_v1_300/`, so the four code-exec tools cannot inspect them by relative workspace paths. The final verifier runs both public and hidden tests.
...
7. 40 samples, two from each generation family, repeat the public/hidden/final checks for stability
